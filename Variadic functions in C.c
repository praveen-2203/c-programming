...
// 1. Calculate the sum of variable number of arguments
int sum (int count, ...) {
    int total = 0;
    va_list args;
...
// 2. Find the minimum element among the variable arguments
int min (int count, ...) {
    va_list args;
    va_start(args, count);
    
    int minimum = va_arg(args, int); // Initialize with the first element
...
// 3. Find the maximum element among the variable arguments
int max (int count, ...) {
    va_list args;
    va_start(args, count);
    
    int maximum = va_arg(args, int); // Initialize with the first element
    for (int i = 1; i < count; i++) {
        int next = va_arg(args, int);
        if (next > maximum) {
            maximum = next;
